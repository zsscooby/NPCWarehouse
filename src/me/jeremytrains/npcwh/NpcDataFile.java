...
public class NpcDataFile {
	...
	private static final int ID = 0, MESSAGE = 1, WORLD = 2, X = 3, Y = 4, Z = 5, YAW = 6, PITCH = 7, NAME= 8, ITEM = 9, LOOKAT = 10, OWNER = 11;
	NPCWarehouse plugin;
	NPCData[] data;
	...
	public void saveNpcData() {
		...
		for (int i = 0; i < data.length; i++) {
			if (data[i] != null) {
				...
				f.format("%s%n", npc.getId() + sep + npc.getMessage() + sep + loc.getWorld().getName() + sep + loc.getX() + sep + loc.getY() + sep + loc.getZ() + sep + loc.getYaw() + sep + loc.getPitch() + sep + npc.npc.getName() + sep + npc.npc.getInventory().getItemInHand().getTypeId() + sep + npc.getLookAt() + sep + npc.getOwner().getName());
			}
		}
		f.flush();
		f.close();
